...
RapSheetCache = RapSheetCache or {}
local MAX_CACHE_SIZE = 500  -- adjustable
local CACHE_EXPIRY_DAYS = 30
...
local function pruneOldCache()
    local now = time()
    for k, v in pairs(RapSheetCache) do
        if now - v.timestamp > (CACHE_EXPIRY_DAYS * 86400) then
            RapSheetCache[k] = nil
        end
    end
end
...
local function enforceCacheLimit()
    local count = 0
    for _ in pairs(RapSheetCache) do count = count + 1 end
    if count > MAX_CACHE_SIZE then
        local sortedKeys = {}
        for k, v in pairs(RapSheetCache) do
            table.insert(sortedKeys, { key = k, ts = v.timestamp })
        end
        table.sort(sortedKeys, function(a, b) return a.ts < b.ts end)
        for i = 1, count - MAX_CACHE_SIZE do
            RapSheetCache[sortedKeys[i].key] = nil
        end
    end
end

local function getCachedData(unitName)
    local data = RapSheetCache[unitName]
    if data and (time() - data.timestamp <= CACHE_EXPIRY_DAYS * 86400) then
        return data.stats
    end
    return nil
end

local function storeToCache(unitName, stats)
    RapSheetCache[unitName] = {
        stats = stats,
        timestamp = time()
    }
    enforceCacheLimit()
...
function RapSheet:INSPECT_ACHIEVEMENT_READY()
    ...
    if GameTooltip:GetUnit() == self.unit then
        ...
        local cached = getCachedData(self.unit)
            if cached then
                self:ShowTooltipFromCache(cached)
                return
            end
...
-- save all variables in stats to cache
        storeToCache(self.unit, stats)
